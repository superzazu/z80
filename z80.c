...
// manually set by the user afterwards.
...
void exec_opcode(z80* const z, u8 opcode) {
    ...
    case 0x8E:
        z->a = addb(z, z->a, rb(z, get_hl(z)), z->cf); break; // adc a,(hl)
    ...
    case 0x9E:
        z->a = subb(z, z->a, rb(z, get_hl(z)), z->cf); break; // sbc a,(hl)
    ...
    case 0xF3: z->iff1 = 0; z->iff2 = 0; break; // di
    ...
    case 0xA7: and(z, z->a); break; // and a
    case 0xA0: and(z, z->b); break; // and b
    case 0xA1: and(z, z->c); break; // and c
    case 0xA2: and(z, z->d); break; // and d
    case 0xA3: and(z, z->e); break; // and e
    case 0xA4: and(z, z->h); break; // and h
    case 0xA5: and(z, z->l); break; // and l
    case 0xA6: and(z, rb(z, get_hl(z))); break; // and (hl)
    case 0xE6: and(z, nextb(z)); break; // and *

    case 0xAF: xor(z, z->a); break; // xor a
    case 0xA8: xor(z, z->b); break; // xor b
    case 0xA9: xor(z, z->c); break; // xor c
    case 0xAA: xor(z, z->d); break; // xor d
    case 0xAB: xor(z, z->e); break; // xor e
    case 0xAC: xor(z, z->h); break; // xor h
    case 0xAD: xor(z, z->l); break; // xor l
    case 0xAE: xor(z, rb(z, get_hl(z))); break; // xor (hl)
    case 0xEE: xor(z, nextb(z)); break; // xor *

    case 0xB7: or(z, z->a); break; // or a
    case 0xB0: or(z, z->b); break; // or b
    case 0xB1: or(z, z->c); break; // or c
    case 0xB2: or(z, z->d); break; // or d
    case 0xB3: or(z, z->e); break; // or e
    case 0xB4: or(z, z->h); break; // or h
    case 0xB5: or(z, z->l); break; // or l
    case 0xB6: or(z, rb(z, get_hl(z))); break; // or (hl)
    case 0xF6: or(z, nextb(z)); break; // or *
...
void exec_opcode_ddfd(z80* const z, u8 opcode, u16* const iz) {
    ...
    case 0xA6: and(z, rb(z, IZD)); break; // and (iz+*)
    case 0xA4: and(z, IZH); break; // and izh
    case 0xA5: and(z, *iz & 0xFF); break; // and izl

    case 0xAE: xor(z, rb(z, IZD)); break; // xor (iz+*)
    case 0xAC: xor(z, IZH); break; // xor izh
    case 0xAD: xor(z, *iz & 0xFF); break; // xor izl

    case 0xB6: or(z, rb(z, IZD)); break; // or (iz+*)
    case 0xB4: or(z, IZH); break; // or izh
    case 0xB5: or(z, *iz & 0xFF); break; // or izl
...
void exec_opcode_cb(z80* const z, u8 opcode) {
    ...
    switch (z_) {
        ...
        case 6: hl = rb(z, get_hl(z)); reg = &hl; break;
...
void exec_opcode_ed(z80* const z, u8 opcode) {
    ...
    case 0x45: case 0x55: case 0x5D: case 0x65: case 0x6D:
    case 0x75: case 0x7D:
        z->iff1 = z->iff2; ret(z); break; // retn
    ...
    case 0x4C: case 0x5C: case 0x6C: case 0x7C: { // neg
        z->a = subb(z, 0, z->a, 0);
    } break;

    case 0x46: case 0x66: z->interrupt_mode = 0; break; // im 0
    case 0x56: case 0x76: z->interrupt_mode = 1; break; // im 1
